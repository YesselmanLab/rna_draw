                            ...
                            vrna_plot_options_puzzler_t *options)
{
  ...
  if ((pair_table) && (x) && (y)) {
    ...
    *y  = (float *)vrna_alloc(sizeof(float) * (length + 1));
    ...
    /*
     * rna_draw: respect a caller-supplied positive budget (set on `options`
     * before this call) instead of unconditionally clobbering it -- only
     * fall back to the 25000 default when the caller left it unset (the
     * NULL-options branch above zero-inits via vrna_alloc()/calloc, so it is
     * 0 there) or explicitly passed a non-positive value.
     */
    if (puzzler->maximumNumberOfConfigChangesAllowed <= 0)
