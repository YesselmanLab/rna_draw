...
def setup_coords_recursive(rootnode, parentnode, start_x, start_y, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE):
    ...
    if(len(rootnode.children_) == 1) :
        ...
        if(rootnode.children_[0].is_pair_): 
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x + go_x * PRIMARY_SPACE,
                                   start_y + go_y * PRIMARY_SPACE, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE)
        elif(rootnode.children_[0].is_pair_ == False and rootnode.children_[0].index_a_ < 0):
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x, start_y, go_x, go_y, NODE_R, PRIMARY_SPACE,
                                   PAIR_SPACE)
        else:   
            setup_coords_recursive(rootnode.children_[0], rootnode, start_x + go_x * PRIMARY_SPACE,
                                   start_y + go_y * PRIMARY_SPACE, go_x, go_y, NODE_R, PRIMARY_SPACE, PAIR_SPACE)
        ...
        for ii in range(0,len(rootnode.children_)):
            ...
            child_x = rootnode.x_ + math.cos(rad_angle) * cross_x * circle_radius + math.sin(rad_angle) * go_x * \
                      circle_radius
            child_y = rootnode.y_ + math.cos(rad_angle) * cross_y * circle_radius + math.sin(rad_angle) * go_y * \
                      circle_radius
            ...
            setup_coords_recursive(rootnode.children_[ii], rootnode, child_x, child_y,
                                   child_go_x / child_go_len, child_go_y / child_go_len, NODE_R, PRIMARY_SPACE,
                                   PAIR_SPACE)
    ...
    def draw(self,offset_x,offset_y,colors, pairs, sequence, render_in_letter, line=False):
        if self.xarray_ != None:
            ...
            if line:
                ...
                if pairs:
                    for pair in pairs:
                        x1, y1 = [offset_x + self.xarray_[pair['from']], offset_y + self.yarray_[pair['from']]], \
                                 [offset_x + self.xarray_[pair['to']], offset_y + self.yarray_[pair['to']]]
                    for pair in pairs:
                        x1, y1 = [offset_x + self.xarray_[pair['from']], offset_y + self.yarray_[pair['from']]], \
                                 [offset_x + self.xarray_[pair['to']], offset_y + self.yarray_[pair['to']]]
                        rec = ConnectionPatch((x1[0],x1[1]),(y1[0],y1[1]), coordsA="data", linewidth=15,
                                              edgecolor='#969696')
                ...
                if not render_in_letter:
                    for ii in range(0,len(self.xarray_)):
                        if colors == None:
                            ...
                            x = self.xarray_[ii] + offset_x
                            y = self.yarray_[ii] + offset_y
                            cir = Circle((x, y), radius=self.NODE_R, facecolor=(colors[ii][0], colors[ii][1],
                                                                                colors[ii][2]), edgecolor='k')
                            self.ax.add_patch(cir)
                if sequence:
                    ...
                    for ii in range(0,len(self.xarray_)):
                        if not render_in_letter:
                            ...
                            text_offset_x = 0
                            text_offset_y = 0
                        self.ax.text(self.xarray_[ii] + offset_x + text_offset_x, 
                                     self.yarray_[ii] + offset_y + text_offset_y, 
                                     sequence[ii], family="monospace", fontsize=text_size, 
                                     ha="center", va="center")
