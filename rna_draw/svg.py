...
class svg(object):
    ...
    def line(self, x1, y1, x2, y2, stroke, width=2):
        ...
        self.__out.write(
            '<line fill="none" stroke="%s" stroke-width="%dpx" x1="%s" y1="%s" x2="%s" y2="%s" x3="0.0" y3="0.0"/>\n'
    ...
    def polygon(self, points, fill, stroke, opacity=1.0):
        ...
        self.__out.write(
            '<polygon fill="%s" stroke="%s" points="%s" opacity="%f"/>\n'
    ...
    def text(self, x, y, size, fill, align, str):
        fill = convert_color(fill)
        self.__out.write(
            ' <text x="%d" y="%d" font-family="sans_serif" font-size="%d" fill="%s" text-anchor="%s">%s</text>'
            % (x - 1.10, y, size, fill, align, str)
        )
        ## rotated
