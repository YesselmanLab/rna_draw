...
class RnaScene(QtWidgets.QGraphicsScene):
    ...
    def build(self, scene: dict, selection: tuple[int, int] | None) -> None:
        ...
        overlaps = set(scene.get("overlaps", []))
        ...
        if len(pos) >= 2:
            ...
            for p in pos[1:]:
                path.lineTo(p)
            bb = self.addPath(path, QtGui.QPen(BACKBONE, 2.0))
            bb.setZValue(-10)
        ...
        for pair in scene.get("pairs", []):
            ...
            if i >= len(pos) or j >= len(pos):
                continue
            color = CROSSING if pair.get("kind") == "crossing" else PAIR
            line = self.addLine(QtCore.QLineF(pos[i], pos[j]), QtGui.QPen(color, 1.6))
            line.setZValue(-5)
        ...
        for routed in scene.get("routed_lines", []):
            ...
            ritem = self.addPath(rpath, QtGui.QPen(ROUTED, 1.6, QtCore.Qt.PenStyle.DashLine))
            ritem.setZValue(-4)
        ...
        for n in nts:
            idx = n["id"]
            center = pos[idx]
            selected = sel_lo is not None and sel_lo <= idx <= sel_hi
            if idx in overlaps:
                fill = OVERLAP_RED
            elif selected:
                fill = SELECT_TEAL
            else:
                fill = QtGui.QColor(n.get("fill", "#9aa0a6"))
            item = QtWidgets.QGraphicsEllipseItem(center.x() - r, center.y() - r, 2 * r, 2 * r)
            edge = SELECT_TEAL if selected else NT_EDGE
            item.setPen(QtGui.QPen(edge, 2.0 if selected else 1.0))
            ...
            if label:
                ...
                text.setBrush(QtGui.QBrush(LABEL))
                font = text.font()
                font.setPointSizeF(max(4.0, r * 0.9))
                text.setFont(font)
...
class RnaGraphicsView(QtWidgets.QGraphicsView):
    ...
    def _rebuild(self) -> None:
        ...
        self._handle = None
        self._scene.build(self._model.scene(), selection)
