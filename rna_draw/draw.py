...
def get_parser():
    ...
    parser.add_argument('-data_vmin', help='min data value everything lower than this will be set to this value',
                        required=False)
    parser.add_argument('-data_vmax', help='max data value everything greater than this will be set to this value',
                        required=False)
    parser.add_argument('-data_ignore_restype',
                        help='data values will be ignored for these restypes, e.g. G and U for DMS', required=False)
    return parser
...
class RNADrawer(object):
    ...
    def __render(self, seq, ss, colors, filename, params):
        ...
        if min(r.xarray_) != max(r.yarray_):
            ...
            r.fig.set_size_inches((max(r.xarray_) - min(r.xarray_)) / ((param[0]) * (area - param[1]) **
                                 (param[2])),(max(r.yarray_) - min(r.yarray_)) / ((param[0]) * (area - param[1]) **
                                 (param[2])))
        r.draw(params.CELL_PADDING, params.CELL_PADDING, colors, pairs, seq, params.RENDER_IN_LETTERS)
...
def __rna_draw_from_args(args):
    ...
    rd = RNADrawer()
    return rd.draw(args.ss, args.seq, args.out, args.color_str, render_type, default_color,
            data)
